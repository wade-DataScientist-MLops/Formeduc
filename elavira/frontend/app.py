...
def send_message_to_api(text):
    try:
        r = requests.post(f"{FASTAPI_BASE_URL}/chat/send_message/", json={"text": text, "user_id": st.session_state.logged_in_user or "Guest"})
        r.raise_for_status()
        return r.json()
    except:
        return None
...
def auth_ui():
    ...
    with col1:
        ...
        if st.button("S'inscrire"):
            if st.session_state.register_new_username and st.session_state.register_new_password:
                try:
                    r = requests.post(f"{FASTAPI_BASE_URL}/users/register/", json={
    ...
    with col2:
        ...
        if st.button("Se connecter"):
            try:
                r = requests.post(f"{FASTAPI_BASE_URL}/users/login/", json={
...
def chat_ui():
    ...
    mic_data = mic_recorder("🎙️ Démarrer", "🛑 Stop", key="mic")
    ...
    for msg in st.session_state.messages:
        role = "assistant" if msg["user_id"] == "Elavira Assistant" else "user"
